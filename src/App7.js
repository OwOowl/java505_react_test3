...
function App7(props) {
  return (
    ...
    // 쿼리스트링 : 기존의 get 방식으로 웹 브라우저의 주소에 파라미터를 함꼐 전달하는 방식
    //  <Route > 컴포넌트의 path에 변경이 없음
    //  지정된 컴포넌트에서 몇 가지 옵션 사항을 사용할 수 있음
    //   옵션 :
    //    pathname : 쿼리스트링을 제외한 현재 주소 경로
    //    search : 맨 앞의 ? 문자를 모함한 쿼리스트링 값
    //    hash : 주소의 # 문자열 뒤의 값 (구형 브라우저에서만 사용)
    //    state : 페이지 이동 시 임의로 추가할 상태값
    ...
    //   navigate(-1) : History를 사용하여 이전 페이지로 이동, 숫자를 음수로 입력 시 해당 이전 페이지 이동이고, 양수 입력 이 앞으로 이동
